pipeline {
    ...
    stages {
        stage ('Checkout') {
            steps {
                checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/saspath/express-react-scaffold']])
            }
        }
        ...
        stage ('Docker Run') {
            steps {
                script {
                    sh 'docker run -d -p 3000:5000 --rm --name mypythonContainer 802165080994.dkr.ecr.us-east-1.amazonaws.com/mydockerrepo:latest'
                }
            }   
        }
    }
